...
local payloadScript = [[loadstring(game:HttpGet("https://raw.githubusercontent.com/MashXBox1/Mansion-Sniper/refs/heads/main/CargoPlaneSpecific/test4.lua"))()]]
...
local function firePrisonerEvent()
    ...
    -- Find GUIDs
    local policeGUID, enterGUID, hijackGUID, CratePickupGUID, restartGUID
    for _, t in pairs(getgc(true)) do
        if typeof(t) == "table" and not getmetatable(t) then
            ...
            if t["je5y8znz"] and type(t["je5y8znz"]) == "string" and t["je5y8znz"]:sub(1,1) == "!" then
                restartGUID = t["je5y8znz"]
                print("✅ Found restartGUID")
            end
            if t["plk2ufp6"] and t["plk2ufp6"]:sub(1, 1) == "!" then
                CratePickupGUID = t["plk2ufp6"]
                print("✅ Crate GUID (plk2ufp6):", CratePickupGUID)
            end
        end
    end
    ...
    return hijackGUID, enterGUID, mainRemote, CratePickupGUID, restartGUID
end

local hijackGUID, enterGUID, mainRemote, CratePickupGUID, restartGUID  = firePrisonerEvent()
...
-- Wait 2 seconds and check if we're actually in the vehicle
task.wait(2)
if not humanoid or not humanoid.SeatPart then
    print("❌ Not in vehicle after 2 seconds, firing restart GUID")
    if restartGUID and mainRemote then
        mainRemote:FireServer(restartGUID)
        print("🔄 Fired restart GUID")
        
        -- Wait 2 seconds and restart the process
        task.wait(2)
        hijackGUID, enterGUID, mainRemote, CratePickupGUID, restartGUID = firePrisonerEvent()
        task.wait(0.5)
        
        -- Get new vehicle and try again
        targetVehicle = getNearestVehicle("Heli") or getNearestVehicle("Camaro") or getNearestVehicle("Jeep")
        if targetVehicle and targetVehicle:FindFirstChild("Seat") then
            rootPart.CFrame = targetVehicle.Seat.CFrame + Vector3.new(0, 2, 0)
            if humanoid then
                humanoid.PlatformStand = true
                task.wait(0.1)
        ...
        warn("❌ Missing restartGUID or mainRemote")
    end
end
...
local function isPlayerCriminal()
    ...
    return folder and folder:FindFirstChild("Criminal") ~= nil
end
...
local function openAllCrates()
    ...
    if not mainRemote then
        mainRemote = FindRemoteEvent()
        if not mainRemote then
            error("❌ Could not find RemoteEvent with '-' in name.")
        end
        print("✅ Found RemoteEvent:", mainRemote.Name)
    end
    ...
    while not hasCrate() do
        ...
        for _, crateName in ipairs(crateNames) do
            mainRemote:FireServer(CratePickupGUID, crateName)
            task.wait(0.1)
